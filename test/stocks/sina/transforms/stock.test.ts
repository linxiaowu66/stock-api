...
describe.skip("【新浪】股票数据转换测试", () => {
  ...
  it("获取股票昨日收盘价", async () => {
    ...
    expect(new SinaStockTransform(SZCode, SZParams).getYesterday()).toBe(
      13466.854
    );
    expect(new SinaStockTransform(HKCode, HKParams).getYesterday()).toBe(
      24710.59
    );
    expect(new SinaStockTransform(USCode, USParams).getYesterday()).toBe(
      26313.6504
    );
  });

  it("获取股票涨跌", async () => {
    expect(new SinaStockTransform(SHCode, SHParams).getPercent()).toBe(
      7.224 / 7.149 - 1
    );
    expect(new SinaStockTransform(SZCode, SZParams).getPercent()).toBe(
      13637.883 / 13466.854 - 1
    );
    expect(new SinaStockTransform(HKCode, HKParams).getPercent()).toBe(
      24595.35 / 24710.59 - 1
    );
    expect(new SinaStockTransform(USCode, USParams).getPercent()).toBe(
      26428.3203 / 26313.6504 - 1
    );
  });

  it("获取股票数据", async () => {
    expect(new SinaStockTransform(SHCode, SHParams).getStock()).toStrictEqual({
      code: "SH510500",
      name: "500ETF",
    ...
    expect(new SinaStockTransform(SZCode, SZParams).getStock()).toStrictEqual({
      code: "SZ399001",
      name: "深证成指",
      percent: 13637.883 / 13466.854 - 1,
      now: 13637.883,
      low: 13399.859,
      high: 13748.034,
      yesterday: 13466.854,
    });
    expect(new SinaStockTransform(HKCode, HKParams).getStock()).toStrictEqual({
      code: "HKHSI",
      name: "恒生指数",
      percent: 24595.35 / 24710.59 - 1,
      now: 24595.35,
      low: 24534.79,
      high: 24938.85,
      yesterday: 24710.59,
    });
    expect(new SinaStockTransform(USCode, USParams).getStock()).toStrictEqual({
      code: "USDJI",
      name: "道琼斯",
      percent: 26428.3203 / 26313.6504 - 1,
      now: 26428.3203,
      low: 26013.5898,
      high: 26440.0195,
      yesterday: 26313.6504,
    });
  });
});
