...
const TencentStockTransform =
  require("stocks/tencent/transforms/stock").default;
...
describe("【腾讯】股票数据转换测试", () => {
  ...
  it("获取股票代码", async () => {
    expect(new TencentStockTransform(code, params).getCode()).toBe("SH510500");
  });

  it("获取股票名称", async () => {
    expect(new TencentStockTransform(code, params).getName()).toBe("500ETF");
  });

  it("获取股票现价", async () => {
    expect(new TencentStockTransform(code, params).getNow()).toBe(7.224);
  });

  it("获取股票最低价", async () => {
    expect(new TencentStockTransform(code, params).getLow()).toBe(7.085);
  });

  it("获取股票最高价", async () => {
    expect(new TencentStockTransform(code, params).getHigh()).toBe(7.28);
  });

  it("获取股票昨日收盘价", async () => {
    expect(new TencentStockTransform(code, params).getYesterday()).toBe(7.149);
  });

  it("获取股票涨跌", async () => {
    expect(new TencentStockTransform(code, params).getPercent()).toBe(
      7.224 / 7.149 - 1
    );
  });
